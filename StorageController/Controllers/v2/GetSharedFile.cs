...
namespace StorageController.Controllers.v2
{
    ...
    public class GetSharedFile : Controller
    {
        ...
        public async Task<string> GetSharedFiles()
        {
            ...
            IEnumerable<UserFile> sharedFiles = db.UserFiles.Where(link => link.UserID == userID && link.Privilege == "Viewer" && db.FileBin.FirstOrDefault(bin => bin.FileID == link.FileID) == null);
        ...
        public async Task<string> GetSharingFiles()
        {
            ...
            IEnumerable<UserFile> sharedFiles = db.UserFiles.Where(link => link.OwnerID == userID && link.Privilege == "Viewer" && db.FileBin.FirstOrDefault(bin => bin.FileID == link.FileID) == null);
            IEnumerable<FileData> sharedFileData = db.Files.Where(link => sharedFiles.FirstOrDefault(shared => shared.FileID == link.FileID) != null);
